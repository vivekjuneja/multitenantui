...
function PhoneListThemeCtrl($scope, $routeParams, Tenant) {
	...
	// Render Tenant Name
	dust.render("test", {
		name : $routeParams.tenant
	}, function(err, out) {
		$scope.tenantName = out;
	});

}
function PhoneListCtrl($scope, $routeParams, Tenant, addToCartService,
		$location) {
	/* Load all tenants */
	...
	if ($scope.tenantselected == "ditto" || $scope.tenantselected == "gsshop") {
		...
		if ($routeParams["category"] == "Electronics"
				|| $routeParams["category"] == "Computers") {
	...
	ten.$get({tenant : $routeParams.tenant }).$promise.then(
		
			function(value) { $scope.tenant = value; },
	...
	*/
	...
	// });
	var self = this;
	// console.log("PhonelistController : get the products from service ---"+
	// addToCartService.productIndex);
	...
	/*
	 * $scope.$on('messageChangeEvent',function(){ console.log("from $on")
	...
	// document.write("Hello ! VIVEK JUNEJA");
}
...
function PhoneDetailCtrl($scope, $routeParams, Product, Tenant,
		addToCartService, $location) {
	$scope.tenantselected = $routeParams.tenant;
	$scope.tenant = Tenant.get({
		tenant : $routeParams.tenant
	});
	...
	/*
	 * console.log("Get the selected products from service --- "+
	 * addToCartService.productIndex);
	 * $scope.$on('messageChangeEvent',function(){ console.log("I am from
	 * handleIdBroadcast of PhoneDetailCntrl---"+addToCartService.productIndex);
	 * });
	 */
...
function shoppingCartStart($scope, $location, $http, addToCartService) {
	...
	// TODO: This is a technical debt :) We are finding out the position the
	// tenant name (enclosed in the url). need to fix this
	...
	$scope.$on('messageChangeEvent', function() {
		console.log("I am from handleIdBroadcast of shoppingCartStart---"
				+ addToCartService.productIndex);
	});

	// console.log($scope.totalProducts.products.length);-- doesnt print because
	// we are making jsonp request
...
PhoneListCtrl.$inject = [ '$scope', '$routeParams', 'Tenant',
		'addToCartService', '$location' ];
shoppingCartStart.$inject = [ '$scope', '$location', '$http',
		'addToCartService' ];
PhoneDetailCtrl.$inject = [ '$scope', '$routeParams', 'Product', 'Tenant',
		'addToCartService', '$location' ];
